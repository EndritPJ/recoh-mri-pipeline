...
def predict_bone_joint_model(n_images: int = 35, radius: int = 32) -> None:
    ...
    try:
        ...
        # Clean up TensorFlow resources to prevent GPU memory accumulation in multi-subject mode
        from keras.backend import clear_session
        clear_session()
        session.close()
        import gc
        gc.collect()
        log.debug('TensorFlow session and resources cleaned up for bone joint prediction')
